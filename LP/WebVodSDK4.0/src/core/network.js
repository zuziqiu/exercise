...
define(function (require) {
  ...
  var network = {
    ...
    setPullInfo: function () {
      ...
      // 桌面分享
      // if(that.room.room.InitData.step7 && that.room.room.InitData.step7.indexOf('pull') > 0) {
      //     // pullInfo = JSON.parse(that.room.room.InitData.step7).c.split("|");
      //     stream = JSON.parse(that.room.room.InitData.step7).streams[0]
      //     pullInfo.push('rtmp://')
      //     pullInfo.push(stream.host)
      //     pullInfo.push(stream.app)
      //     pullInfo.push(stream.path)
      //     that.pullHostType.desktop = pullInfo;
      // }
      // // 课件
      // if(that.room.room.InitData.step3 && that.room.room.InitData.step3.indexOf('pull') > 0) {
      //     // pullInfo = JSON.parse(that.room.room.InitData.step3).c.split("|");
      //     stream = JSON.parse(that.room.room.InitData.step3).streams[0]
      //     pullInfo.push('rtmp://')
      //     pullInfo.push(stream.host)
      //     pullInfo.push(stream.app)
      //     pullInfo.push(stream.path)
      //     that.pullHostType.video = pullInfo;
      // }
      // // 教育模式
      // if(core.currentMode == core.constant.MODE_EDUCATION){
      //     pullInfo = that.pullHostType.video;
      // }
      // // 视频 & 课件模式
      // else{
      //     pullInfo = that.pullHostType.desktop;
      // }
      // // 设置 pullinfo 信息
      if (pullInfo) {
    ...
    setDefinition: function (definition) {
      ...
      if (that.groupData[that.sourceSetting.sourceName]) {
        ...
        var streamExt = '';
        if (stream.definition && stream.definition[that.curDefinition]) {
