...
var player = {
  ...
  setting: function (type, pack) {
    ...
    //     // 表情包
    //     case 'emotions':
    //         if(pack && typeof pack.base === 'string' && pack.maps && typeof pack.maps === 'object'){
    //             that._setting.emotions = pack
    //         }
    //         break;
    //     default:
    //         break;
  ...
  resizePlayer: function (w, h) {
    ...
    return whiteboard.resize(w, h, function () {
      // 1.Resize
      // 2.从当前时间点取点播数据
      // 3.重绘
      that.resizeTimer = setTimeout(function () {
        that.getCommandsByDuration(that.currentDuration, function (cmds) {
          that.execute(cmds);
        });
      }, 50);
    });
  ...
  mainPlayer: function (containerId, playerId, callback) {
    ...
    Store.commit(TYPES.UPDATE_VIDEO_PLAYER_DOM, {
      wrapContainer: containerId,
      playerId: playerId,
  ...
  camera: function (containerId, playerId, callback) {
    Store.commit(TYPES.UPDATE_CAMERA_PLAYER_DOM, {
      wrapContainer: containerId,
      playerId: playerId,
      vodCallback: callback
    })
  ...
  changeMedia: function (tagName, isLock) {
    ...
    if (!tagName) {
      return false;
    } else if (!tagName.match(/(video|audio)/ig)) {
      return false;
    }
    ...
    that.loadJson(url, function (commands) {
      ...
      for (var i = 0, pagelength = pages.length; i < pagelength; i++) {
        ...
        if (pages[i].page) {
          ...
          if (pages[i].page.p > 10000) {
            thumbUrl = STATIC.STATIC_HOST + '/open/cooperation/default/vod-pc/css/img/error.png';
          } else {
      ...
      if (that.cameraSectionsV2) {
        ...
        for (var i = 0; i < camlist.length; i++) {
          //补全
          if (lastCamType == core.COMMAND.CAMERA_START && camlist[i].t != core.COMMAND.CAMERA_STOP) {
            TempCamObj.type = "camera";
            TempCamObj.et = parseFloat(camlist[i].st - 0.01);
            reBuildCamera.push(TempCamObj);
            TempCamObj = {};
            lastCamType = core.COMMAND.CAMERA_STOP;
          }

          if (camlist[i].t == core.COMMAND.CAMERA_START) {
            TempCamObj.st = parseFloat(camlist[i].st);
            lastCamType = core.COMMAND.CAMERA_START;
          } else if (camlist[i].t == core.COMMAND.CAMERA_STOP) {
            TempCamObj.type = "camera";
        ...
        //补全
        if (lastCamType != core.COMMAND.CAMERA_STOP) {
          TempCamObj.type = "camera";
      ...
      for (var c = 0; c < that.cameraSectionsV2.length; c++) {
        var cameraObjs = {};
        cameraObjs["type"] = core.COMMAND.CAMERA_START;
        cameraObjs["time"] = that.cameraSectionsV2[c].st;
        nativeCameraSections.push(cameraObjs);

        var cameraObjs = {};
        cameraObjs["type"] = core.COMMAND.CAMERA_STOP;
  ...
  vodCommandFilter: function (command) {
    ...
    if (command && command.c) {
      ...
      if (t == core.COMMAND.PAGE && content[2] == 3) {
        flag = true;
      }
    } else {
      flag = false;
  ...
  dispatch: function (command) {
    ...
    // 批量命令
    if (command.t == core.COMMAND.DRAW_LIST) {
      var jsonCommand;
      // 移动端要进行数据迭代
      if (tools.isMobile()) {
  ...
  execute: function (command) {
    ...
    if (tools.in_array(command.t, [
      ...
      core.COMMAND.TEXT,
      core.COMMAND.IMAGE_PRELOAD,
      core.COMMAND.STOP,
      core.COMMAND.START,
      core.COMMAND.DRAW_LIST,
      core.COMMAND.RUBBER
    ])) {
      // H5点播
      // if (tools.isMobile()) {
      ...
      // else {
      //   // 特殊命令过滤
      //   // if(that.vodCommandFilter(command)){
      //   //     return false;
      //   // }else{

      //   // PC指令暴露
      //   that.pcCommandExprose(command);

      //   // 渲染
      //   that.getVodSwfPlayer(function (player) {
      //     tools.debug("flash execute ===>", command);
      //     player.command(JSON.stringify(command));
      //   });
      // }
    }
    // } catch (err) {
    //   throw new Error("Player 指令错误===>: " + JSON.stringify(command));
    // }
  ...
  pcCommandExprose: function (command) {
    // 翻页
    if (command.t == core.COMMAND.PAGE) {
        ...
        for (i = 0; i < commands.length; i++) {
          for (j = 0; j < commands[i].datas.length; j++) {
            if (commands[i].datas[j]) {
              if (commands[i].datas[j].t == core.COMMAND.VIDEO_START) {
                commands[i].datas[j].c = that.live.videoUrl.shift();
              } else if (commands[i].datas[j].t == core.COMMAND.DESKTOP_START) {
  ...
  getCommandsByDuration: function (_duration, callback) {
    ...
    // that.clear(targetPage.page.p) // echo
    ...
    if (targetPage) {
      ...
      // 优先渲染当前页
      // that.dispatch(targetPage.page)
      whiteboard.seekPage(targetPage.page, () => {
        // 加载draw文件
        that.loadDraw(closeSetPage.i, function (i) {
          ...
          for (var k = 0, klen = cmds.length; k < klen; k++) {
            ...
            if (parseInt(cmds[k].st) <= _duration) {
              ...
              // 纯桌面分享不用执行指令集
              if (that.playMode == core.constant.MODE_DESKTOP) {
                // 桌面分享不处理指令
              } else {
                that.isSkip = false;
                ...
                // if (!tools.isMobile()) {
                if (cmds[k].t == core.COMMAND.DRAW_LIST) {
  ...
  getAvatar: function (data) {
    ...
    if (!avatar) {
      ...
      avatar = STATIC.STATIC_HOST + "/open/cooperation/default/live-pc/css" + img;
    }
    return avatar;
  },
  // 获取问答数据
  ...
  getModeType: function (currentMode) {
    ...
    if (currentMode === "desktop") {
      mode.currentMode = core.constant.MODE_DESKTOP;
      mode.beforeMode = core.constant.MODE_EDUCATION;
    }

    // 视频
    else if (currentMode === "video") {
      mode.currentMode = core.constant.MODE_EDUCATION;
      mode.beforeMode = core.constant.MODE_DESKTOP;
    }
  ...
  seekCore: async function (duration) {
    ...
    var that = this,
      ...
      //duration取整
      playMode = core.constant.MODE_EDUCATION,
      //当前模式
      beforeMode = core.constant.MODE_EDUCATION; //旧模式
    ...
    // 桌面分享处理
    else if (playMode == core.constant.MODE_DESKTOP) {
      var ds = currentVideoSection.detail;
      var config = Store.getters('getExtData').config
      var inspectUrl = null
      var isM3U8 = /\.m3u8/ig.test(ds.allUrl[1]);
      // if (config && config.techOrder == 'HLS' && !that.Hls) {
      if (isM3U8 && !that.Hls) {
    ...
    // 上一个模式：教育模式
    // 当前模式：桌面分享
    if (beforeMode == core.constant.MODE_EDUCATION) {
      if (video.cameraPlayer) {
        tools.debug("摄像头暂停中...");
    ...
    // 上一个模式：桌面分享模式
    // 当前模式：教育直播
    else if (beforeMode == core.constant.MODE_DESKTOP) {
  ...
  getDuration: function (callback, timerInterval) {
    ...
    if (!that.isFirstPlay) {
      if (
        tools.isWechat() &&
        that.currentMediaPath === 0 &&
        video.playStatus === "ended" || videoPlayer.playStatus === "ended"
      ) {
    ...
    // 单视频
    if (that.videoCurType === "video") {
      if (video.playStatus === 'playing') {
        that.currentDuration = that.partStartTime + video.currentDuration;

      } else if (video.playStatus === "ended" || video.playStatus === "changing") {
        that.currentDuration += timerInterval;

        // that.currentDuration += that.partStartTime;
      } else if (video.playStatus === "seeking" || video.playStatus === "waiting") {
  ...
  setCurVideo: async function (duration, isSeek) {
    ...
    if (vData.detail.name === "video") {
      core.currentMode = core.constant.MODE_EDUCATION;
    } else if (vData.detail.name === "desktop") {
      core.currentMode = core.constant.MODE_DESKTOP;
    }
    ...
    if (vData.detail.name === "video") {
      ...
      // 处理无缝视频连接, 提前 1秒 收起视频
      if (video.cameraPlayer) {
        let videoTimeLeft = (video.cameraPlayer.duration() - video.cameraPlayer.currentTime());
      ...
      var isM3U8 = /\.m3u8/ig.test(inspectUrl);
      // if (config && config.techOrder == 'HLS' && !that.Hls) {
      if (isM3U8 && !that.Hls) {
        // await import ('hls.js').then(Hls => {})
        await import('hls.js').then(res => {
  ...
  cmdsRun: function (duration) {
    var that = this;
    tools.long("cmdruns ==> ", parseInt(duration, 10), that.runPageIndex, that.pagesNum, that.curPageIndex);
    for (var i = that.runPageIndex; i < that.pagesNum; i++) {
      ...
      tools.long("cmdruns page data ==> ", that.commandStore[that.curPageIndex]);
  ...
  tick: function () {
    ...
    if (!that.tickTimmer) {
      ...
      that.tickTimmer = setInterval(function () {
        ...
        // logs
        tools.long(
          'currentDuration:==>' + that.currentDuration +
          '\n initDuration:==>' + schedule.initDuration +
          '\n video.playStatus:==>' + video.playStatus +
          '\n desktop.playStatus:==>' + videoPlayer.playStatus +
          '\n action:==>' + that.action
        );
