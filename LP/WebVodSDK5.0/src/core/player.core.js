...
var core = {
  ...
  getFlvVideoUrl: function (stream) {
    return STATIC.URL.PROTOCOL + this.stream2flv(stream)
  },
  ...
  getVideoUrl: function (mainStream) {
    ...
    // 直播
      ...
      // m3u8地址组装
      urlStream = STATIC.URL.PROTOCOL + that.stream2m3u8(content);
  ...
  stream2flv: function (content) {
    ...
    urlStream = content.hosts.flv + content.domain +
      "/" + content.app +
      "/" + content.path + content.ext.flv + (content.query.length > 0 ? "?" + query : "");
    tools.debug('get flv url ==>', STATIC.URL.PROTOCOL + urlStream)
    return urlStream;
  },
  // remove play icon
  ...
  //     return false
  //   }
  //   let conf = sdkStore.getState().room.extConfig
