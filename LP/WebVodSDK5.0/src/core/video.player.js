...
sdkStore.listen(
  ...
  (dispatch, cur, prev, state) => {
    if (mainPlayer.desktopVideoDom) {
      mainPlayer.desktopVideoDom.playbackRate = cur
    }
  }
)
const mainPlayer = {
  ...
  on(eventName, payload) {
    console.debug('videoPlayer emit on ==>', eventName, payload)
    switch (eventName) {
      // create
      case 'video:player':
        this.init()
  ...
  // video-player
  getPlayer: function () {
    var playerCtx = sdkStore.getState().player
    ...
    if (!element) {
      ...
      // 创建桌面分享元素
      if (sdkStore.getState().room.extConfig.config?.audioOnly) {
  ...
  start: async function (command, seek) {
    ...
    let playerCtx = sdkStore.getState().player
    ...
    if (!that.player) {
      ...
      var isM3U8 = /\.m3u8/gi.test(videoUrl)
  ...
  setMeidaPlayStatus: function (state) {
    var that = this
    console.debug('video on event ===> ' + state, 'desktop')
    // 抛出事件
