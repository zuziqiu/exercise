...
export class Arrow extends graphicBase {
  ...
  flush (data) {
    ...
    // console.warn(this.store.getState().drawId)
    // this.store.dispatch({
    //   type: TYPE.DRAW_LINE_ID,
    //   payload: this.store.getState().drawId.line++
    // })
  }
  // 默认设置
  defaultSetting () {
  ...
  toCdata (line) {
    ...
    let cdata = {
      ...
      color: tools.color(whiteboard.strokeColor),
  ...
  mouseDown (data) {
    ...
    if (!this.onPress) {
      ...
      this.mainLine.id = 'l' + this.store.getState().drawId
      this.defaultSetting()
