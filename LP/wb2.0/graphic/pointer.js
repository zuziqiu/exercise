...
export class Pointer extends graphicBase {
  ...
  createKlss({ data }) {
    ...
    if (data) {
      ...
      this.pointer.hex = this.tools.color(this.store.getState().whiteboard.strokeColor)
      this.pointer.id = curUserId + '_' + new Date().getTime() + '_' + 'c' + this.store.getState().drawId
