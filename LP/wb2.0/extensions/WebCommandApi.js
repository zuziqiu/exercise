...
export const WebCommandApi = class Api {
  ...
  reciveFromCommand(targetData) {
    ...
    if (Object.prototype.toString.call(targetData) == '[object String]') {
      tools.log('Recive Qt Data ===> ' + targetData)
      targetData = JSON.parse(targetData)
    } else {
      tools.log('Recive Qt Data ===> ' + JSON.stringify(targetData))
    }
    // try {
    //   targetData = JSON.parse(targetData)
    // } catch (err) {
      ...
      if (targetData.type) {
        ...
        // 此处判断data很重要。没有数据表示是自主创建，有数据表示是来源于外部的数据（图片和文字比较特殊，自主创建也会携带data:{src, fontSize}。图片还要判断isSend,文字还要判断data中的t字段）
          ...
          if (
            ...
            STATIC.TRIANGLE == targetData.type
          ) {
          ...
          if (targetData.type == STATIC.IMAGE) {
            ...
            if (targetData.data.effect == 1) {
              let wb = globalStore.getState().WHITEBOARD,
                keys = Object.keys(wb),
                lastKey = [...keys].pop(),
                target = {
                  key: "LIVE_SET_PAGE",
              ...
              let _Page = new Page()
              _Page.setPageDispatcher(target)
            } else {
