/**
 * 课件模式（PPT图片模块
 */
import React, { PureComponent } from 'react';
import { observer } from 'mobx-react';
import { toJS } from 'mobx'
import wbEmitter from '../extensions/wbEmitter'
// base
import { globalStore } from '../states/globalStore'
import * as TYPES from '../Action/action-types'
import * as tools from '../extensions/util'
import { STATIC } from '../states/staticState'
import { addListener } from 'resize-detector'
import { hostMachine } from '../extensions/hostMachine'

@observer
export class PPT extends PureComponent {
  // init.
  constructor() {
    super()
    this.state = {
      isShowImg: false,
      isPPTMode: false,
      pptImgWrapId: 'ht-ppt-loader',
      pptImgId: 'ppt-img-main',
    }
    this.scroll_lock = true
    this.localScale = null
    // this.resizeInit = false
    this.postTop = 0
    // 虚拟dom的id数组
    this.prestrainDomId = []
    this.on()
  }
  // 监听emit
  on() {
    wbEmitter.on(TYPES.PPT_RESIZE, () => {
      tools.log('wbEmitter receive ==> imgResize')
      // ppt的父级容器
      this.renderDom()
      // 图片resize
      this.imgResize()
      this.imgPosition()
    })
  }
  // 监听store
  listener() {
    // ppt图片地址（所有切换的动作）
    globalStore.listen(
      () => { return { img: toJS(globalStore.store.cousewareResource.img) } },
      ({ img }) => {
        // 切换src的时候清空淡入淡入的类名，让图片立刻消失
        if (document.querySelector(`#${this.state.pptImgId}`)) document.querySelector(`#${this.state.pptImgId}`).className = ''
        // 静态PPT切换或者翻页时触发loading提示
        if (globalStore.store.room.pptType == 0) {
          wbEmitter.emit('wb:usePPT:status', { status: false })
        }
        // 预加载
        // this.prestrainImg()
        // 资源超时5S重试
        hostMachine.process({
          time: 5000,
          oriUrl: img,
          type: 'timeOut',
          callback: function (res) {
            // UPDATE_DOC_IMG
            globalStore.actions.dispatch('cousewareResource', {
              type: TYPES.UPDATE_DOC_IMG,
              payload: res
            })
          }
        })
      }
    )
    // resize
    globalStore.listen(
      () => { return { ratio: toJS(globalStore.store.pptInfoResource.ratio) } },
      (ratio) => {
        wbEmitter.emit('ppt:resize')
        // 对外暴露PPT比例
        wbEmitter.emit('ppt:ratio', { ratio: ratio })
      }
    )
    // 监听涂鸦权限判断是否可以滚动
    globalStore.listen(
      () => { return { powerEnable: toJS(globalStore.store.room.powerEnable) } },
      ({ powerEnable }) => {
        if (powerEnable) {
          // 学员身份不允许滚动(没有身份时并且有涂鸦权限可以滚动，兼容新版直播器)
          if (!(globalStore.store.room.curUser && globalStore.store.room.curUser.role === 'user')) {
            globalStore.actions.dispatch('ppt', {
              type: TYPES.IS_SCROLL_PPT,
              payload: 'true'
            })
          }
        } else {
          globalStore.actions.dispatch('ppt', {
            type: TYPES.IS_SCROLL_PPT,
            payload: 'false'
          })
        }
      }
    )
    /* 
      此处是检测观看端记录的滚动位置，观看端PPT[pages]数据不完整，被添加翻页时pages会改变。
      另外所有端切换ppt时peges会变，但受限于postTop不会进行计算 
    */
    globalStore.listen(
      () => { return { pages: toJS(globalStore.store.ppt.pages) } },
      ({ pages }) => {
        if (pages[globalStore.store.page.currentPage].postTop) {
          if (!document.querySelector(`#${this.state.pptImgId}`)) {
            return false
          }
          let img = document.querySelector(`#${this.state.pptImgId}`)
          let getInspectData = this.getInspectData(img)
          // 讲师上课前PPT滚动的话，观看端要之前把接收的postTop转化为当前页面所在比例中的滚动并执行
          // this.postScale = 800 / naturalWidth
          // this.localScale = parentWidth / naturalWidth
          // 以postTop = 100为例，curPagePPT['postTop'] / this.postScale ：即先算出在800容器中每滚动100所对应的真实图片滚动的高度
          // curPagePPT['postTop'] / this.postScale * this.localScale ：除法算出真实图片滚动的高度后换算成在父级容器的页面中的滚动高度
          let localScale = getInspectData._parent.width / getInspectData.IMG_DOM.width
          let top = parseInt(pages[globalStore.store.page.currentPage].postTop / this.postScale * localScale)
          // 更新滚动的top
          this.updateScrollTop(img, top)
        }
      }
    )
  }
  // mount
  componentDidMount() {
    this.listener()
    this.eventTrigger()
    // this.fire()
  }
  // update
  componentDidUpdate() {
    // 切换画板和翻页的时候 ratio不能被监听变化，所以要在这里更新父级容器宽度
    // this.renderDom()
  }
  // img error
  imgOnerror(img) {
    // 资源错误重试
    hostMachine.process({
      time: 0,
      oriUrl: img.target.src,
      type: 'error',
      callback: function (res) {
        // UPDATE_DOC_IMG
        globalStore.actions.dispatch('cousewareResource', {
          type: TYPES.UPDATE_DOC_IMG,
          payload: res
        })
      }
    })
  }
  // 取dom
  getInspectData(img) {
    let _parentDom = document.querySelector(`#${this.state.pptImgWrapId}`)
    if (!_parentDom) {
      return false
    }
    let _parent = {
      width: _parentDom.clientWidth,
      height: _parentDom.clientHeight
    }
    let IMG_DOM = {
      width: img.naturalWidth,
      height: img.naturalHeight
    }
    let ratio = IMG_DOM.height / IMG_DOM.width
    this.postScale = STATIC.WHITEBOARD_BASE_WIDTH / IMG_DOM.width
    return {
      _parent: _parent,
      IMG_DOM: IMG_DOM,
      ratio: ratio,
    }
  }
  // img laod
  imgOnload(event) {
    // 控制淡入淡出的类名
    event.target.className = 'transition fade-in'
    // 请求成功后清空资源重试的计时器
    hostMachine.clear(event.target.src)
    let imgTarget = event.target
    // if (imgTarget) {
    // 翻页后要发送的滚动top置0
    this.postTop = 0
    let getInspectData = this.getInspectData(imgTarget),
      pptInfoResource = {
        width: getInspectData.IMG_DOM.width,
        height: getInspectData.IMG_DOM.height,
        ratio: getInspectData.ratio,
        scale: this.localScale,
        postScale: this.postScale,
        loaded: 'true',
      };

    // 更新图片属性记录
    globalStore.actions.dispatch('pptInfoResource', {
      type: TYPES.PPT_INFO_SOURCE,
      payload: {
        pptInfoResource: pptInfoResource
      }
    })
    this.renderDom()
    // 计算图片的适应尺寸
    this.imgResize(imgTarget)
    // 计算图片的适应位置
    this.imgPosition(imgTarget)
    // 可能存在翻页回来有滚动过的情况
    let isSendFromImg = imgTarget.src.indexOf('isSend=1') > 0 ? 1 : 0

    /* 
     * isSendFromImg:图片解析出来的isSend && 画板dispatch的isSend(换PPT的时候，判断该值) 
     * 当前页面可能存在 (isSendFromImg) && (state.page.isSend)对应以下的情况：
     * 1、小班应用PPT (1) && (undefined)
     * 2、直播器主播端主动应用 (1) && (1)
     * 2、直播器主播端被动应用 (0) && (0)
     * 3、直播器嘉宾端主动应用 (1) && (1)
     * 3、直播器嘉宾端被动应用 (0) && (0)
     * 特殊情况：直播器主播主动应用(isSendFromImg=1)=>被动应用=>切换画板(0)=>PPT重新渲染(isSendFromImg==1)。这个流程中isSendFromImg首先记录是1，PPT重新渲染会触发flush，所以依靠切换画板(0)的状态更新state.page.isSend=0限制发送
     */
    if (isSendFromImg != 0 && globalStore.store.page.isSend != 0) {
      // 翻页的操作码发0
      this.flush(0)
    }
    this.imgComplete()
    // }
  }
  imgResize(img) {
    // 当被window.resize触发的时候，传入的两个参数都不存在
    img = img || document.querySelector(`#${this.state.pptImgId}`)
    if (img) {
      let getInspectData = this.getInspectData(img)
      img.style['width'] = `${Math.floor(getInspectData._parent.width)}px`
      img.style['height'] = `${Math.floor(getInspectData._parent.width * getInspectData.ratio)}px`
    }
  }
  imgPosition(img) {
    // 当被window.resize触发的时候，传入的两个参数都不存在
    img = img || document.querySelector(`#${this.state.pptImgId}`)
    if (img) {
      let getInspectData = this.getInspectData(img)
      let currentPage = globalStore.store.page.currentPage
      let pages = globalStore.store.ppt.pages
      let top = 0
      // 大于ppt默认高/宽比例，此时ppt比例 >= 9/16
      if (getInspectData.ratio >= STATIC.PPT_RATIO) {
        if (img.height > Math.round(getInspectData._parent.height)) {
          // 图片渲染完成（可以获取到图片信息）
          // 学员身份不允许滚动(没有身份时并且有涂鸦权限可以滚动，兼容新版直播器)
          if (!(globalStore.store.room.curUser && globalStore.store.room.curUser.role === 'user')) {
            this.pptWheel(img);
            // 此时ppt比例（高/宽）> 3/4 ,可以滚动并且出现滚动指示器
            if (getInspectData.ratio > STATIC.RATIO) {
              globalStore.actions.dispatch('ppt', {
                type: TYPES.IS_SCROLL_PPT,
                payload: globalStore.store.room.powerEnable ? 'true' : 'false'
              })
            }
          }
          if (getInspectData.IMG_DOM.width) {
            this.localScale = getInspectData._parent.width / getInspectData.IMG_DOM.width
            let MODE = tools.isWhiteboard(currentPage) ? STATIC.WHITEBOARD : STATIC.PPT
            let curPagePPT = pages[currentPage]
            // 最新的curPagePPT在助教端会存在、在学生端不会存在
            if (curPagePPT) {
              if (curPagePPT['postTop']) {
                // 讲师上课前PPT滚动的话，观看端要之前把接收的postTop转化为当前页面所在比例中的滚动并执行
                // this.postScale = 800 / naturalWidth
                // this.localScale = parentWidth / naturalWidth
                // 以postTop = 100为例，curPagePPT['postTop'] / this.postScale ：即先算出在800容器中每滚动100所对应的真实图片滚动的高度
                // curPagePPT['postTop'] / this.postScale * this.localScale ：除法算出真实图片滚动的高度后换算成在父级容器的页面中的滚动高度
                top = parseInt(curPagePPT['postTop'] / this.postScale * this.localScale)
              } else {
                if (curPagePPT['scale']) {
                  // 本地翻页把之前保存的scale和可能进行过缩放的窗口计算重新赋值top
                  top = parseInt(top / (curPagePPT['scale'] / (getInspectData._parent.width / getInspectData.IMG_DOM.width)))
                } else {
                  // postTop和scale都不存在（原始值）
                  top = 0
                }
              }
            } else {
              // 设置localScale前先更新top,先取curPagePPT['scale']是因为适配翻页，没有翻页时用localScale
              top = parseInt(top / (this.localScale / (getInspectData._parent.width / getInspectData.IMG_DOM.width)))
            }
            // 设置localScale
            // this.localScale = getInspectData._parent.width / getInspectData.IMG_DOM.width
            if (curPagePPT) {
              if (!curPagePPT['scale'] || curPagePPT.scale != this.localScale) {
                globalStore.actions.dispatch('ppt', {
                  type: TYPES.UPDATE_PPT_PAGE_SCALE,
                  payload: {
                    mode: MODE,
                    page: currentPage,
                    scale: this.localScale
                  }
                })
              }
            }
          }
          // 更新滚动位置
          this.updateScrollTop(img, top)
        }
        // 此时 3/4 > ppt比例（高/宽）> 9/16，仍然在3/4容器中上下居中
        else {
          // 上下居中
          // 注意危险！！！top != 0 时-top附加偏移数值。常规情况进入这里的逻辑top == 0。特别注意兼容旧版直播器用960尺寸时会有一个怪异偏移值。在这里需要附加到图片上，但是涂鸦不用附加，我也是醉了。
          if (top != 0) {
            img.style['margin-top'] = (getInspectData._parent.height - img.height) / 2 - top + 'px'
          } else {
            // 此判断内图片高度不足父容器高度，所以设居中显示，canvas的top设0
            img.style['margin-top'] = (getInspectData._parent.height - img.height) / 2 + 'px'
          }
          // canvas的top设0
          document.querySelectorAll('canvas').forEach(function (item, index) {
            item.style.top = '0px'
          })
          if (globalStore.store.ppt.isScrollPPT === 'true') {
            globalStore.actions.dispatch('ppt', {
              type: TYPES.IS_SCROLL_PPT,
              payload: 'false'
            })
          }
        }
      } else {
        // 重写PPT（16:9）时图片属性和定位
        // 需要铺满图片的容器
        let container = document.querySelector('#canvas-container-inner')
        let containerData = {
          width: container.clientWidth,
          height: container.clientHeight
        }
        if (containerData.height / containerData.width > getInspectData.ratio) {
          // 宽度占满容器
          img.style['width'] = containerData.width + 'px'
          img.style['height'] = containerData.width * getInspectData.ratio + 'px'
          img.height = containerData.width * getInspectData.ratio
        } else {
          // 高度占满容器
          img.style['height'] = containerData.height + 'px'
          img.style['width'] = containerData.height / getInspectData.ratio + 'px'
          img.width = containerData.height / getInspectData.ratio
        }
        // 没有足够高度的图片取消滚动
        if (globalStore.store.ppt.isScrollPPT === 'true') {
          globalStore.actions.dispatch('ppt', {
            type: TYPES.IS_SCROLL_PPT,
            payload: 'false'
          })
        }
        // 此判断内图片高度不足父容器高度，所以设居中显示，canvas的top设0
        img.style['margin-top'] = (getInspectData._parent.height - img.height) / 2 + 'px'
      }
    }
  }
  // 静态图片加载完成暴露对外
  imgComplete() {
    if (globalStore.store.room.pptType == 0 || globalStore.store.room.pptType == 1) {
      wbEmitter.emit('wb:usePPT:status', { status: true })
    }
  }
  // 当前页起上下两页预加载
  prestrainImg() {
    let page = globalStore.store.page
    let curPage = page.currentPage + '_' + page.currentSubPage
    let curIndex = page.pageComputed.findIndex(item => item === curPage)
    if (page.pageComputed[curIndex - 2]) {
      let targetPage = page.pageComputed[curIndex - 2].split('_')[0]
      let targetSubpage = page.pageComputed[curIndex - 2].split('_')[1]
      this.prestrainDom(page.pptId, targetPage, targetSubpage)
    }
    if (page.pageComputed[curIndex - 1]) {
      let targetPage = page.pageComputed[curIndex - 1].split('_')[0]
      let targetSubpage = page.pageComputed[curIndex - 1].split('_')[1]
      this.prestrainDom(page.pptId, targetPage, targetSubpage)
    }
    if (page.pageComputed[curIndex + 1]) {
      let targetPage = page.pageComputed[curIndex + 1].split('_')[0]
      let targetSubpage = page.pageComputed[curIndex + 1].split('_')[1]
      this.prestrainDom(page.pptId, targetPage, targetSubpage)
    }
    if (page.pageComputed[curIndex + 2]) {
      let targetPage = page.pageComputed[curIndex + 2].split('_')[0]
      let targetSubpage = page.pageComputed[curIndex + 2].split('_')[1]
      this.prestrainDom(page.pptId, targetPage, targetSubpage)
    }
  }
  prestrainDom(id, page, subPage) {
    if (!this.prestrainDomId.includes(id + '_' + page + '_' + subPage)) {
      this.prestrainDomId.push(id + '_' + page + '_' + subPage)
      let img = document.createElement('img')
      img.src = tools.getPPTImg(page, subPage)
    }
  }
  computingScroll(direction, img) {
    // 记录PPT滚动的位置UPDATE_PPT_PAGE_POST_TOP
    let curPage = globalStore.store.page.currentPage
    if (globalStore.store.ppt.pages[curPage].postTop) {
      this.postTop = parseInt(globalStore.store.ppt.pages[curPage].postTop)
    }
    let operationCode = null
    let scrollTop = img.offsetTop
    let baseInertval = 100
    let interval = parseInt(baseInertval * this.localScale / this.postScale) // 800宽度窗口滚动100，本质为800与页面图片的父容器宽度比
    // 课件naturalWidth在STATIC.RATIO比例中减去页面窗口高度后可以滚动的高度
    let naturalScrollHeight = img.naturalHeight * this.postScale - 600
    if (direction === 'up') {
      if (img.offsetTop < 0) {
        // 最后一次滚动的高度不够时以实际为准
        // 最后一次naturalScrollHeight不够时以实际为准
        if (interval > Math.abs(img.offsetTop)) {
          interval = Math.abs(img.offsetTop)
          baseInertval = naturalScrollHeight - Math.floor(naturalScrollHeight / 100) * 100
        } else {
          baseInertval = 100
        }
        // 本地滚动的top
        scrollTop += interval
        // 发送的top
        this.postTop += baseInertval
        operationCode = 6
        this.scroll_lock = false
      } else {
        this.scroll_lock = true
      }
    }
    if (direction === 'down') {
      if (img.clientHeight - document.querySelector(`#${this.state.pptImgWrapId}`).clientHeight > Math.abs(img.offsetTop)) {
        // 剩余可以滚动的高度
        let offsetVal = img.clientHeight - document.querySelector(`#${this.state.pptImgWrapId}`).clientHeight - Math.abs(img.offsetTop)
        // 最后一次滚动的高度不够时以实际为准
        if (interval > offsetVal) {
          interval = offsetVal
        }
        // 本地滚动的top
        scrollTop -= interval
        // 发送的top
        this.postTop -= baseInertval
        // 最后一次naturalScrollHeight不够时以实际为准
        if (this.postTop < -naturalScrollHeight) {
          this.postTop = -naturalScrollHeight
        }
        operationCode = 5
        this.scroll_lock = false
      } else {
        this.scroll_lock = true
      }
    }
    // 滚动不锁定时执行滚动
    if (!this.scroll_lock) {
      // 更新发送的top
      this.updatePostTop(this.postTop)
      // 更新滚动的top
      this.updateScrollTop(img, scrollTop)
      // todo set handlerxid(新版直播器没有curUser,此处需要判断兼容)
      let _xid = globalStore.store.room.curUser ? globalStore.store.room.curUser.xid : null
      if (_xid) {
        globalStore.actions.dispatch('room', {
          type: TYPES.UPDATE_ROOM_HANDLER_XID,
          payload: _xid
        })
      }
      this.flush(operationCode)
    }
  }
  // updatePostTop
  updatePostTop(postTop) {
    // 记录PPT滚动的位置UPDATE_PPT_PAGE_POST_TOP
    let curPage = globalStore.store.page.currentPage
    // 这里存储的是会发送的postTop
    globalStore.actions.dispatch('ppt', {
      type: TYPES.UPDATE_PPT_PAGE_POST_TOP,
      payload: {
        page: curPage,
        postTop: postTop
      }
    })
  }
  // updateScrollTop,滚动和翻页都必须走这里。翻页重新设置是因为可能拉伸了页面再进行翻页
  updateScrollTop(img, top) {
    // 滚动到锚点(把top分开处理是因为兼容图片高度不足的时候的情况，此时canvas还是4:3)
    let imgTop = top
    let canvasTop = top
    if (Math.round(document.querySelector(`#${this.state.pptImgWrapId}`).clientHeight) > img.height) {
      // 注意危险！！！top != 0 时-top附加偏移数值。常规情况进入这里的逻辑top == 0。特别注意兼容旧版直播器用960尺寸时会有一个怪异偏移值。在这里需要附加到图片上，但是涂鸦不用附加，我也是醉了
      if (top && top != 0) {
        imgTop = (document.querySelector(`#${this.state.pptImgWrapId}`).clientHeight - img.height) / 2 - top
      } else {
        imgTop = (document.querySelector(`#${this.state.pptImgWrapId}`).clientHeight - img.height) / 2
      }
      canvasTop = 0
    }
    this.scrollToAnchor(img, imgTop, canvasTop)
  }
  scrollToAnchor(img, imgTop, canvasTop) {
    // 记录当前的滚动位置
    // this.postTop = top
    img.style['margin-top'] = `${imgTop}px`
    document.querySelectorAll('canvas').forEach(function (item, index) {
      item.style.top = `${canvasTop}px`
    })
    // 计算指示器状态,顶部或者底部不可滚动时指示器箭头变色
    if (document.querySelector(".scroll_up")) {
      if (parseInt(imgTop) === 0) {
        document.querySelector(".scroll_up").classList.remove('scroll_direction')
      } else {
        document.querySelector(".scroll_up").classList.add('scroll_direction')
      }
    }
    if (document.querySelector(".scroll_down")) {
      if (img.clientHeight === document.querySelector(`#${this.state.pptImgWrapId}`).clientHeight + Math.abs(img.offsetTop)) {
        document.querySelector(".scroll_down").classList.remove('scroll_direction')
      } else {
        document.querySelector(".scroll_down").classList.add('scroll_direction')
      }
    }
  }
  // 封装指令
  flush(operationCode) {
    let webCommand = globalStore.getCommandApi()
    let cmdTpl = tools.getFlushData({ type: 'ppt', operationCode: operationCode })
    // 发送Qt
    webCommand && webCommand.sendToQt(cmdTpl)
    // 发送操作（发给小班的，小班可能会也可能不会post，判断xid）
    tools.log('ppt doFlush ==>', cmdTpl)
    wbEmitter.emit('set:page', cmdTpl)
  }
  // PPT滚动相关事件
  pptWheel(img) {
    let that = this
    // 滚动滑轮触发滚动PPT
    document.querySelector('#talkfun-client-whiteboard').onmousewheel = function (e) {
      if (globalStore.store.room.powerEnable) {
        //判断浏览器IE，谷歌滑轮事件
        if (e.wheelDelta) {
          //当滑轮向上滚动时
          if (e.wheelDelta > 0) {
            that.computingScroll('up', img)
          }
          //当滑轮向下滚动时
          if (e.wheelDelta < 0) {
            that.computingScroll('down', img)
          }
        } else if (e.detail) {
          //Firefox滑轮事件
          //当滑轮向上滚动时
          if (e.detail > 0) {
            that.computingScroll('up', img)
          }
          //当滑轮向下滚动时
          if (e.detail < 0) {
            that.computingScroll('down', img)
          }
        }
      }
    };

    // 点击按钮触发滚动PPT
    document.querySelector('#talkfun-ppt-loader-wrap').onclick = function (e) {
      if (globalStore.store.room.powerEnable) {
        if (e.target.classList.contains('scroll_up')) {
          that.computingScroll('up', img)
        }
        if (e.target.classList.contains('scroll_down')) {
          that.computingScroll('down', img)
        }
      }
    }
  }
  renderDom() {
    let dom = document.querySelector('#' + this.state.pptImgWrapId)
    let parentDom = document.querySelector(`#${globalStore.store.room.wbContainerId}`)
    if (dom) {
      let sourceWidth = parentDom.clientWidth
      let _width = parentDom.clientWidth
      let _height = null
      let scrollHeight = parentDom.clientHeight
      // > 3:4 的PPT比例
      if (globalStore.store.pptInfoResource.ratio > STATIC.PPT_RATIO) {
        _height = parentDom.clientWidth * STATIC.RATIO
        if (_height > scrollHeight) {
          _width = scrollHeight / STATIC.RATIO
          _height = scrollHeight
        }
      } else {
        let _container = document.querySelector('#canvas-container-inner')
        let containerData = {
          width: _container.clientWidth,
          height: _container.clientHeight
        }
        let _containerRatio = containerData.height / containerData.width
        if (_containerRatio > STATIC.PPT_RATIO) {
          _width = sourceWidth
          _height = sourceWidth * STATIC.RATIO
        } else {
          // 算出高度占满情况下的原4:3时候的父级盒子宽度
          let cache_width = scrollHeight / STATIC.RATIO
          // 用高度占满情况下的原4:3时候的父级盒子宽度算出16:9时候的PPT高度
          let cache_height = cache_width * STATIC.PPT_RATIO
          // 算出占满的高度与16:9时候PPT高度之比
          let cache_ratio = scrollHeight / cache_height
          _width = cache_width * cache_ratio
          _height = _width * STATIC.RATIO
        }
      }
      let _leftOffset = (sourceWidth - _width) / 2
      dom.style['width'] = _width + 'px'
      dom.style['height'] = _height + 'px'
      dom.style['margin-left'] = _leftOffset + 'px'
      dom.style['margin-top'] = (scrollHeight - _height) / 2 + 'px'
      dom.style['position'] = 'absolute'
      dom.style['top'] = '0'
      dom.style['overflow'] = 'hidden'
    }
  }
  eventTrigger() {
    if (document.querySelector(`#${globalStore.store.room.wbContainerId}`)) {
      addListener(document.querySelector(`#${globalStore.store.room.wbContainerId}`), (res) => {
        tools.log('window resize', `#${globalStore.store.room.wbContainerId}`)
        // wbEmitter.emit('ppt:resize')
        wbEmitter.emit(TYPES.PPT_RESIZE)
      })
      window.addEventListener('resize', function () {
        wbEmitter.emit(TYPES.PPT_RESIZE)
      }, false)
    }
  }
  isLoadImg() {
    let img = globalStore.store.cousewareResource.img
    if (img) {
      return true
    } else {
      return false
    }
  }
  // object or array
  render() {
    let curPage = globalStore.store.page.currentPage
    if (tools.isWhiteboard(curPage) || curPage == 0) {
      return ''
    }
    // render
    return (
      <div id="talkfun-ppt-loader-wrap">
        <div id={this.state.pptImgWrapId} className="ppt-img-loader">
          {this.isLoadImg() ? (
            <img id={this.state.pptImgId} src={globalStore.store.cousewareResource.img} onLoad={this.imgOnload.bind(this)} onError={this.imgOnerror.bind(this)} />
          ) : null}
        </div>
        {globalStore.store.ppt.isScrollPPT == 'true' && globalStore.store.room.nativeGraphic != 1 ? (
          <div className="scroll_button">
            <div className="scroll_up"></div>
            <div className="scroll_down scroll_direction"></div>
          </div>
        ) : null}
      </div>
    )
  }
}