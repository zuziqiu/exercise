...
export class Curve extends graphicBase {
  constructor({ data }) {
    ...
    this.onPress = false
    this.id = this.store.room.drawId
    if (data) {
      this.render(data)
    }
    let canvas = this.fabric
    let setting = this.store.wbProperty
  ...
  pathCreate(klssPath) {
    ...
    if (_path) {
      ...
      _path.xid = this.store.room.curUser ? this.store.room.curUser.xid : '',
        _path.hex = this.tools.color(this.store.wbProperty.strokeColor)
  ...
  render(data) {
    ...
    if (cspliter) {
      ...
      this.tools.log('curve pathData undefined')
    }
