...
export class Text extends graphicBase {
  constructor({ data, fromClick, existKlss }) {
    super()
    this.onPress = false
    this.Text = null
    this.disposer = {
    ...
    this.timingControl = {
      text: null, // 用于对比长时间聚焦状态却没有修改时不需要发送
      input: 0, // 文字input的状态
      interval: null // 定时器
    }
    this.render({ data, fromClick, existKlss })
  }
  // 发送数据
  flush(data) {
    // 有数据才发送
  ...
  // 默认设置
  defaultSetting(klss) {
    klss.fill = this.store.wbProperty.strokeColor
    klss.stroke = this.store.wbProperty.strokeColor
    klss.strokeLineCap = this.store.wbProperty.strokeLineCap
    // klss.strokeWidth = this.store.wbProperty.strokeWidth
  ...
  toCdata(text) {
    ...
    let cdata = {
      ...
      color: this.tools.color(text.stroke),
  ...
  createKlss({ data, fromClick }) {
    ...
    if (fromClick && !editingFlag) {
      new Promise((reslove, reject) => {
        ...
        if (data && data.target && data.target.isText) {
          ...
          new Promise((reslove, reject) => {
            ...
            let fontSize = this.store.wbProperty.brushData.fontSize
            this.Text = new fabric.IText('', {
              ...
              hasControls: false,
              visible: true,
              fill: this.store.wbProperty.strokeColor,
              stroke: this.store.wbProperty.strokeColor,
              strokeLineCap: this.store.wbProperty.strokeLineCap,
              selectable: true,
              selection: true,
              hasborder: false,
              isText: true,
            ...
            this.Text.set({
              ...
              id: curUserId + '_' + new Date().getTime() + '_' + 't' + id,
              nid: id,
      ...
      if (!that.store.room.curUser) {
        _editable = true
      } else if (_data.x == that.store.room.curUser.xid) {
        // 是自身创建的文字，假如powerEnable为true就允许编辑
        _editable = that.store.room.powerEnable
      } else {
        // 非自身创建的文字不允许编辑
        _editable = false
      }
      ...
      that.Text = new fabric.IText(unescape(_data.c.text), {
        // 当前画笔类型是文字时设置可以移动的属性
        lockMovementX: that.store.wbProperty.brushType === STATIC.TEXT ? false : true,
        lockMovementY: that.store.wbProperty.brushType === STATIC.TEXT ? false : true,
        lockRotation: true,
        lockScalingFlip: true,
        lockScalingX: true,
        lockScalingY: true,
        ...
        strokeLineCap: that.store.wbProperty.strokeLineCap,
        // 当前画笔类型是文字时设置可以选中的属性
        selectable: that.store.wbProperty.brushType === STATIC.TEXT ? true : false,
        selection: that.store.wbProperty.brushType === STATIC.TEXT ? true : false,
        hasborder: false,
        isText: true,
        editable: _editable,
        fontFamily: 'Microsoft YaHei',
  ...
  // input时开始定时发送
  timingInspect(target) {
    if (this.timingControl.input == 1) {
      if (!this.timingControl.interval) {
        this.timingControl.interval = setInterval(() => {
          if (this.timingControl.text != target.text) {
            this.flush(target)
            this.timingControl.text = target.text
          }
        }, 450)
      }
    }
  ...
  // 渲染
  render({ data, fromClick, existKlss }) {
    // _toSVG(待考察方法)
  ...
  mouseDown(data) {
    ...
    this.firstPoint.Y = p.y
    // 文字类型并且非编辑状态才解锁拖拽的属性
    if (data.target && data.target.isText && !data.target.isEditing) {
      data.target.set({
        lockMovementX: false,
        lockMovementY: false,
      })
  ...
  mouseUp(data) { }
  /*
   * 只有修改或者移动才会进入objectModified
  */
  objectModified(data) {
    // 移动事件触发会有transform字段
    // 只有移动才允许flush，修改的flush在onInput中处理了
    if (data.transform) {
      // 移动发送数据
      this.flush(data.target)
    } else {
      // 重置定时器对象的属性
      this.timingControl.input = 0
      clearInterval(this.timingControl.interval)
      this.timingControl.text = null
      this.timingControl.interval = null
    }
  }
  onInput(data) {
    // 输入状态时给字体大小设置拦截器
    if (!this.disposer.fontDisposer) {
      // 修改字体大小的时候退出当前编辑对象并发送
      this.disposer.fontDisposer = intercept(globalStore.store.wbProperty.brushData, "fontSize", change => {
        // exitEditing会触发objectModified，在objectModified会执行发送
        this.Text.exitEditing()
        // 取消拦截器
        this.disposer.fontDisposer()
        // 返回修改后的值
        return change;
      })
    }
    // 输入状态时给涂鸦颜色设置拦截器
    if (!this.disposer.colorDisposer) {
      // 修改涂鸦颜色的时候退出当前编辑对象并发送
      this.disposer.colorDisposer = intercept(globalStore.store.wbProperty, "strokeColor", change => {
        // exitEditing会触发
        this.Text.exitEditing()
        // 取消拦截器
        this.disposer.colorDisposer()
        // 返回修改后的值
        return change;
      })
    }
    // 设置input状态
    this.timingControl.input = 1
    this.timingInspect(data.target)
  }
}