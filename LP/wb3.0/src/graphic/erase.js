...
export class Erase extends graphicBase {
  constructor({ data, isClearAll }) {
    ...
    if (isClearAll) {
      ...
      this.flush(
        {
          x: this.store.room.curUser ? this.store.room.curUser.xid : '',
          st: '',
          c: '',
          n: '',
          p: this.store.page.currentPage,
          t: 1
        },
        'eraseAll'
      )
  ...
  toCdata(klss, type) {
    ...
    } else if (klss.tid === this.STATIC.RECTANGLE || klss.tid === this.STATIC.ELLIPSE || this.STATIC.TRIANGLE) {
      cdata.postCdata = {
        id: klss.id,
        visible: 0,
        x1: 0,
  ...
  // curUser
  curUser(xid) {
    let _curUser = this.store.room.curUser
    // curUser 为null时即为纯画板
    if (!this.store.room.curUser) return true
  ...
  onSelectionCreated(data) {
    ...
    if (data.target && data.target.id) {
      if (this.curUser(data.target.xid)) {
        ...
        webCommand && webCommand.sendToQt(postCdata)
        // history.save(saveCdata)
        wbEmitter.emit(this.TYPES.UPDATE_PAGE_DRAW_DATA, { drawData: saveCdata })
        wbEmitter.emit('draw:data', postCdata)
      }
    }
  }
  // down
