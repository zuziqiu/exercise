...
export class Image extends graphicBase {
  constructor({ data: data, existKlss: existKlss }) {
    super()
    this.onPress = false
    this.onMove = false
    this.Image = null
    // if (arguments[0]) {
    this.render({ data: data, existKlss: existKlss })
    // }
  }
  // 发送数据
  flush(data) {
    let cdata = Object.assign({}, this.toSerializable(data))
    delete cdata.klss
    this.tools.log('image flush ==>', cdata)
    super.flush(cdata)
  }
  // 默认设置
  defaultSetting(image) {
    super.defaultSetting(image)
  }
  // 发送给远端
  toServer() {
    // todo...远程接口
  ...
  toCdata(image) {
    ...
    // let wbProperty = globalStore.sotre.wbProperty
  ...
  toSerializable(klss) {
    ...
    // let curUserId = globalStore.sotre.room.curUser ? globalStore.sotre.room.curUser.xid : 'me'
    cmd.x = baseTpl.x
    cmd.st = baseTpl.st
    cmd.n = klss.nid || baseTpl.n
    cmd.cid = klss.id
    cmd.hd = baseTpl.hd
    cmd.p = globalStore.store.page.currentPage || baseTpl.p
    cmd.t = STATIC.IMAGE
  ...
  createKlss({ data, isFlush }) {
    let that = this
    let curUserId = globalStore.store.room.curUser ? globalStore.store.room.curUser.xid : 'me'
    ...
    if (data) {
      ...
      imageSrc = globalStore.store.wbProperty.brushData.src
      serverPath = globalStore.store.wbProperty.brushData.server_path
      // 设置没有数据时图片默认属性
      that.Image = {
        id: `${curUserId}_${new Date().getTime()}_i${globalStore.store.room.drawId}`,
        nid: globalStore.store.room.drawId,
        xid: globalStore.store.room.curUser ? globalStore.store.room.curUser.xid : '',
        tid: that.STATIC.IMAGE,
        visible: true,
        isPicture: true,
        optType: 1
    ...
    //支持webp的，改为加载web
    tools.webpSupport(function () {
      if (window.qt) {
        // 直播器里面才有qt对象，直播器是直接引用本地地址 => file:///E:.........
        that.createImgObj(imageSrc, serverPath, data, isFlush)
      } else {
        // 非直播器，一般是常规浏览器，需要取线上地址https://........
        imageSrc = serverPath
  ...
  setImage(imgObj, data) {
    ...
    if (data) {
      ...
      if (_data.c.visible == '0') {
        ...
        graphicTempObject.clear('imgList', imgObj.id)
        // 设为在画板上不可见状态
        imgObj.set({
          visible: false,
        })
  ...
  render(objData) {
    ...
    // optType: cArray[3],
    // matrix: cArray[4],
    // existKlss 用于讲师端双击图片重新实例化当前图片类时不渲染（true时不渲染）（为了取flush等属性方法,另外klss属性已经在双击事件里赋值）
    if (!objData.existKlss) {
      if (objData.data) {
