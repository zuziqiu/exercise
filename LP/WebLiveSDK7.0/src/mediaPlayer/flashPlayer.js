...
const flashPlayer = {
  ...
  play(stream) {
    ...
    let mode = sdkStore.room.curMode
  ...
  on(eventName, payload) {
    ...
    if (eventName === 'camera:player') {
      ...
      const camera = sdkStore.media.player.camera
    ...
    if (eventName === 'video:player' || eventName === 'ppt:player') {
      ...
      let wpDom = sdkStore.media.player.video
  ...
  mainPlayer: function (containerId, playerId, callback) {
    ...
    if (!flashResult.flash) {
      tools.warn('PPT播放器加载失败, 请安装或允许Flash插件 (https://get.adobe.com/cn/flashplayer/)')
      tools.warn('Chrome 启用 Flash Player 方法 ==> (https://www.flash.cn/help/article/article.php?id=45)')
      eventStore.emit('flash:load:error', flashResult)
      return false
    }
  ...
  camera: function (containerId, playerId, callback) {
    ...
    let initData = sdkStore.room.initData
    var isShowDebug = (location.href.indexOf("sdkDebug=list") > 0) ? 'list' : '',
      swfVersionStr = "10.2.0",
  ...
  cameraCallback: function (cameraPlayer) {
    ...
    var handle = function () {
      ...
      if (that._cameraPlayerLoaded) {
        clearInterval(timer);
        eventStore.emit('cameraCallback', cameraPlayer)
    ...
    var live = {
      ...
      livePlayerLoaded: function () {
        ...
        livePlayer.getPlayer(function (_livePlayer) {
          // 避免不刷新情况下重复加载
          eventStore.emit('livePlayerCallback', _livePlayer)
      ...
      playerLoaded: function () {
        ...
        parent.getPlayer(function (mainPlayer) {
          ...
          eventStore.emit('mainPlayerCallback', mainPlayer)
          parent.mainPlayerCallback();
        });
      ...
      desktopFirstPlay: function () {
        ...
        eventStore.emit('live:media:play', 'desktop')
      },
      // Camera 开始播放
      cameraFirstPlay: function () {
        tools.debug('camera fplay')
        eventStore.emit('live:media:play', 'camera')
      },
      // 摄像头播放开始(音频、画面)
      ...
      flashAnnounce: function (params) {
        ...
        eventStore.emit('player:announce', params)
        return params;
      }
    }
    return live
  ...
  destroy() {
    ...
    const mainPlayer = sdkStore.media.player.video
