...
export default {
  ...
  commonEventListener: function (videoEl) {
    ...
    videoEl.addEventListener('x5videoenterfullscreen', function () {
      ...
      eventStore.emit('wx:enter:fullscreen')
    }, false)
    // 退出全屏
    videoEl.addEventListener('x5videoexitfullscreen', function () {
      ...
      eventStore.emit('wx:exit:fullscreen')
    }, false)
    // 是否断网
    window.addEventListener('online', event => {
      mediaControler.emit('live:network:online', 'online')
    }, false)
    window.addEventListener('offline', event => {
      mediaControler.emit('live:network:offline', 'offline')
    }, false)
  },
  ...
  bindEvent(vdom, playType) {
    ...
    el.on("loadeddata", function () {
      ...
      eventStore.emit('live:camera:loadeddata')
      eventStore.emit('live:video:loadeddata')
    }, false);
    ...
    el.on("play", function () {
      ...
      eventStore.emit('live:video:play')
      // 播放>2's获取时间修正
      if (shakeLock) {
    ...
    el.on("pause", function () {
      ...
      eventStore.emit('live:camera:pause')
    ...
    el.on("abort", function () {
      ...
      eventStore.emit('live:camera:abort')
      eventStore.emit('live:video:abort')
    ...
    el.on("error", function (err) {
      ...
      eventStore.emit('live:camera:error', code)
      eventStore.emit('live:video:error', code)
      ...
      // HLS-直接更换CDN(在线)
      setTimeout(() => {
        mediaControler.emit('video:on:error', this)
      }, 1500)
    }, false);
    ...
    el.on("ended", function () {
      ...
      eventStore.emit('live:camera:ended')
      eventStore.emit('live:video:ended')
      that.loggerCore('ended', true)
      mediaControler.emit('video:on:ended', this)
    }, false);
    ...
    el.on("suspend", function () {
      ...
      eventStore.emit('live:camera:suspend')
      eventStore.emit('live:video:suspend')
    ...
    el.on("stalled", function () {
      ...
      // that.bn += 1;
      // that.ba += 1;
      eventStore.emit('live:video:stalled')
    ...
    el.on("waiting", function () {
      ...
      // that.bn += 1;
      // that.ba += 1;
      eventStore.emit('live:camera:waiting')
      eventStore.emit('live:video:waiting')
    ...
    el.on("timeupdate", function () {
      ...
      // 更新播放状态
      if (sdkStore.media.player.status !== 'playing') {
        sdkAction.dispatch('media', {
          type: TYPES.UPDATE_MEDIA_STATUS,
          payload: {
            status: 'playing'
      ...
      // 大于0开始计时
      if (curTime > 0) {
        eventStore.emit('live:camera:timeupdate', curTime)
        eventStore.emit('live:video:timeupdate', curTime)
        // 计算是否timeout
        that.timeoutChecker('timeupdate')
        // 正常播放解锁
        that.timeoutLock = false
      }
      cmdSchedule.setPlayDuration(curTime)
    }, false);
  },
  ...
  timeoutChecker: function (flag) {
    ...
    let liveState = sdkStore.room.liveState
    if (liveState !== 'start') {
      ...
      that.roomIncreaseTimer = setInterval(function () {
        ...
        if (localDelay > that.PLAY_TIMEOUT_LIMIT) {
          if (!that.timeOutTimes) {
            ...
            // 切换源
            mediaControler.emit('live:video:timeout', localDelay)
            setTimeout(() => {
              that.timeOutTimes = false
            }, 200)
          }
          ...
          // timeout-cmd
          eventStore.emit('live:camera:timeout', localTime, videoTime)
          eventStore.emit('live:video:timeout', localTime, videoTime)
          eventStore.emit('live:media:timeout', localDelay)
        }
        // 卡顿自动递增
    ...
    tools.ajax({
      ...
      success: function (retval) {
        ...
        if (retval.code == STATIC.CODE.SUCCESS) {
          cmdSchedule.set('playDuration', 0)
          cmdSchedule.set('initDuration', retval.data.duration)
          cmdSchedule.set('baseDuration', retval.data.duration)
          tools.debug("getServerCurTime ==> initDuration:" + retval.data.duration)
        }
        // 已停止播放(让直播停止)
        else if (retval.code == 1202) {
          mediaControler.emit('live:on:stop', 'global')
        }
      }
    });
  },
  ...
  destroy(type, callback) {
    ...
    if (vdom?.parentNode) {
      ...
      // 删除元素Icon
      mediaControler.mdeidaOnModeChange()
      // 重置videoDom对象
      mediaControler.setVideoDom(null)
      // 删除vdom
