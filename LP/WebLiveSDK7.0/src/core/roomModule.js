...
let room = {
  ...
  init: function (access_token, callback) {
    ...
    // 第一次请求初始化数据时执行生命周期函数，重试不执行
    // lifeCycle.lifeControler['beforeInterface']?.next()
    // 请求数据
    this.getInitData()
  },
  // 请求initData
  getInitData: function () {
    ...
    tools.ajax({
      ...
      success: function (retval) {
        ...
        // 大班低延迟模式
        if (retval.room?.modetype == STATIC.LOWDELAY_MODE) {
          // 请求 appid 相关
        ...
        if (retval.code === STATIC.CODE.SUCCESS) {
          ...
          // 房间callback
          try {
            eventStore.emit('live:init:success')
            _ts.initCallback(room)
          } catch (err) {
            console.error('初始化 callback出错 ==>', err)
          }
        }
        // code 课程范围[1260-1263]未允许进入
        else if (STATIC.CODE.LIVE_COURSE_NOT_START >= retval.code && retval.code >= STATIC.CODE.LIVE_COURSE_UNEXIST) {
          eventStore.emit('live:course:access:error', retval)
        }
        // 系统错误信息
        else {
          eventStore.emit('system:room:error', retval)
        }
      },
      // 请求错误
      error: function (res) {
        ...
        if (_ts.ajaxState !== 'success') {
          ...
          eventStore.emit('system:room:error', "room请求超时,请重试...")
        }
      }
    })
  },
