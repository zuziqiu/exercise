    ...
    function ajaxs(options) {
      ...
      if (dataType === 'jsonp') {
        ...
        s.onerror = function () {
          ...
          throw new Error('TalkFunMessage script onerror: 请求发送失败')
        }

        // 前置参数
        if (defaults.beforeSend?.() === false) {
          return false
          s = null
        }
        dhead.appendChild(s)
      ...
      // 跨域script
        ...
        s.src = url
        s.onload = function () {
          defaults.success?.(this)
          s = null
        }
        s.onerror = function () {
          defaults.error?.({ message: 'script loaded error' })
          s = null
      ...
      // XMLHttpRequest
        ...
        // POST请求
          ...
          if (defaults.beforeSend?.(xhr) === false) {
            xhr = null
            return false
          }
