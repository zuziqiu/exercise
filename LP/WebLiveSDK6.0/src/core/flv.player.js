...
var flvPlayer = {
   ...
   */
  fixTimeOffset(res) {
    let curLiveTime = schedule.getCurLiveTime()
    let course = Store.getters('getGlobal')['data']['course']
    let curLastTartget = 0
    if (course && course.offset && course.offset.length > 0) {
  ...
  flvEvent: function (player) {
    ...
    player.on(flvjs.Events.SCRIPTDATA_ARRIVED, function (res) {
      ...
      // onTextData
      if (res.onTextData && res.onTextData.text) {
        res = getDisposeTimeObj('onTextData', res.onTextData.text)
        schedule.setDelayTime(res)
      }
      // SEI
      if (typeof res === 'string') {
        if (res.indexOf('disposeTime') > -1) {
          // 设置时间类型
          schedule.set('timeType', 'SEI')
          // 同步时间
      ...
      // [注意]: ==> 不能删除
      setTimeout(() => {
        if (that.curVdom && that.curVdom.buffered && that.curVdom.buffered.length > 0) {
          ...
          if (delay > that.MAX_DELAY_RELOAD) {
            ...
            schedule.set('cmdDelay', flvDelay)
          }
        }
      }, 150)
      if (res.type) {
  ...
  play: function (playOnly) {
    if (this.videoPlayer) {
      // 播放逻辑
      playerCore.elementPlay(this.curVdom).then(res => {
  ...
  createPlayer: function (type, vdom) {
    ...
    if (type === 'video' && !this.videoPlayer) {
      ...
      tools.debug('FLVJS-VERSION ==>', flvjs.version)
  ...
  getFlvUrl: function (stream) {
    ...
    // this.flvUrl = playerCore.getFlvVideoUrl(stream)
