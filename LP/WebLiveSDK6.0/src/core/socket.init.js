...
import cmdHandle from './cmd.handle'
...
let socket = {
	...
	isGid: function (packet) {
		let me = Store.getters('getInitData')['user']
		// 自己是 => 0 不过滤 || 接收端 => 0 不过滤
		if (me && me.gid === 0 || packet && packet.args && packet.args.gid === 0) {
			tools.debug('On Public Gid ==> ' + me.gid, packet.args.gid)
			return true
		}
		// 过滤聊天
		else if (packet.cmd && packet.cmd === 'chat:send') {
			if (packet.args && packet.args.gid > 0) {
				let me = Store.getters('getInitData')['user']
				tools.debug('Socket on Gid ==>', 'me ==>' + me.gid, 'recive ==>' + packet.args.gid)
				if (me && me.gid === packet.args.gid) {
					return true
				} else {
					return false
				}
	...
	filters: function (packet) {
		...
		if (packet.cmd.indexOf('third:') > -1) {
			...
			map.get('third:broadcast')(packet)
		} else {
			switch (packet.cmd) {
				// 虚拟用户
				case 'member:robots':
				...
				case 'chat:send':
					if (packet.args) {
						...
						if (conf && conf.config.filterChat) {
	...
	logger: function (type, exts) {
		let user = Store.getters('getInitData')['user']
		let logData = {
			uri: this.obj.io.uri,
	...
	reConnect: function (connect_error_times) {
		...
		if (connect_error_times > 3) {
			try {
				...
				tools.error(`[connect_error]socket链接失败 => ${err}`)
			}
		}
	},
	// 连接服务器
