...
var map = function () {
	...
	this.put = function (key, value) {
		...
		if (window.location.href.indexOf("mapkey") > -1) {
			tools.debug("MAP--PUT ===> " + key);
		}
 ...
 * @return {Object} Object
	...
	this.get = function (key, isUnfix) {
		if (window.location.href.indexOf("maplog") > -1) {
			tools.debug("MAP--GET ===> " + key);
			tools.warn((this.data[key]))
		}
		if (window.location.href.indexOf("mapkey") > -1) {
			tools.warn("MAP--GET ===> " + key);
		}
		if (isUnfix) {
		...
		return function () {
			if (this.dataMap[key] && this.dataMap[key].length > 0) {
				this.dataMap[key].forEach(target => {
