...
define(function (require) {
	...
	var tools = {
		...
		detectProtocol: function (content) {
			return content.replace(/http:\/\//ig, window.location.protocol + "//");
		},
		...
		_typeof: function (obj) {
			...
			return (_type.replace(/object /, '').replace(/^\[|\]$/g, '')).toLowerCase()
		},
		...
		getSDKMode: function () {
			...
			// 模式2: ppt h5 + 原生
			// 暂未开通
			var _match = window.location.href.match(/sdkmode=[0-9]/ig),
				curMode = 0;
			// 取地址最后传入模式
		...
		isShowDebug: function () {
			return window.location.href.indexOf("debug=list") > -1;
		},
		// small_v1
		...
		// 长log
		long: function () {
			if (location.href.indexOf('debug=long') > -1) {
		...
		getGuid: function () {
			...
			if (!that.guid) {
				if (window.localStorage) {
					...
					that.guid = navigator.userAgent.toLowerCase().substr(30, 36).replace(/[^\w\-]/g, '-');
				}
			}
