...
// static path
seajs.protocol = window.location.protocol + "//";
seajs.domain = seajs.protocol + window.mainConfig.resHost;
seajs.resPath = seajs.domain + "";

// 获取 `sdkVersion` 版本
window.sdkVersion = window.sdkVersion || "2.6";
...
seajs.baseConfig = {
    ...
    // 地址相关
    mainDomain: seajs.domain,
    debugMode: "sdkVersion=test",
    
    // 正式包
    sdkPath: "//" + window.TALKFUN_STATIC_HOST + "/open/maituo_v2/dist/vod/sdk-vod.{sdkVersion}.min.js",

    // JS目录相关
    jsPath: seajs.resPath + "/js",
    jsDebugPath: seajs.resPath + "/js/debug",
    commonPath: seajs.resPath +"/"
};
...
if(seajs.deploy && seajs.isdebug){
    ...
    seajs.weinre = seajs.baseConfig.weinre;
}
// 外网正式版
...
seajs.config({
    ...
    //别名配置
    alias: {
        'jquery'       : seajs.baseConfig.commonPath + 'common/vendor/jquery/jquery-3.1.0.min',
        '$'            : seajs.baseConfig.commonPath + 'common/vendor/jquery/jquery-loader',
        'zepto.animate': seajs.baseConfig.commonPath + 'common/vendor/zepto/zepto.animate',
        "zepto.data"  : seajs.baseConfig.commonPath + 'common/vendor/zepto/zepto.data',
        'dropload'    : seajs.baseConfig.commonPath + 'common/vendor/dropload/zepto.dropload',
        "TMOD"        : seajs.baseConfig.commonPath + "common/vendor/tmod-cmd/tmod.min",
        "nouislider"  : seajs.baseConfig.commonPath + "common/vendor/nouislider/nouislider.min", 
        "scrollTo"    : seajs.baseConfig.commonPath + "common/vendor/scroll-to/jquery.scrollTo.min", 
        "sly"         : seajs.baseConfig.commonPath + "common/vendor/sly/sly.min",
        // ==== SDK包 ====
        "SDK"         : seajs.sdkPack
    },
    ...
    // 预加载
    preload: [
        // seajs.isdebug ? seajs.weinre : null, //是否启用weinre模块调试
    	"jquery",
        "nouislider",
    ],
...
if(seajs.config){
    var d = new Date(),
        TIME_STAMP = "?v="+d.getYear().toString()+d.getMonth().toString()+d.getDate().toString();
