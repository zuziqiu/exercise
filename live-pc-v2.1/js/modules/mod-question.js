    ...
    var modQuestion = {
        ...
        bindEvents: function(){
            var that = this,
                ...
                // 助教管理盒子
                $adminPopBox = $("#admin_pop_box");
            // 问答管理列表的回复
            $("body").on('click', function(e){
                    // 问答管理列表的发送按钮
                var $comfrimQuestionPost = $(".comfirm_question_post"),
                    // 问答管理列表的textarea
                    $questionPostDetail = $("#question_post_detail"),
                    // qid
                    qid = "",
                    // 取到发送的状态
                    _target = $(e.target);
                if(_target.hasClass("question_reply")){
                    var $rid = $(e.target).data("rid"),
                        $nickname = $(e.target).data("nickname");
                    $comfrimQuestionPost.attr("data-type", "reply");
                    $comfrimQuestionPost.attr("data-rid", $rid);
                    $questionPostDetail.attr("placeholder", "回答 "+$nickname+"的问题:");
                    $questionPostDetail.attr("data-rid", $rid).focus();
                }else{
                    // 点击回复后会聚焦输入框，此时点击输入框时维持状态
                    if(_target.parent().parent().attr("class") != "question_post"){
                        // 重置状态
                        $questionPostDetail.attr("placeholder", "请输入文字...");
                        $comfrimQuestionPost.attr("data-rid", "0");
                        $comfrimQuestionPost.attr("data-type", "ask");
                        $questionPostDetail.removeAttr("data-rid");
                    }
                };
                // 问答列表textarea keydown
                // $questionPostDetail.on("keydown", function(e){
                //     if(e.keyCode === 13){
                //         that.emitQuesManagement();
                //         e.preventDefault();
                //         e.stopPropagation();
                //         return false;
                //     }
                // });
                // 问答列表发送按钮 click
                if(_target.hasClass('comfirm_question_post')){
                    that.emitQuesManagement();
                    e.preventDefault();
                    e.stopPropagation();
                    return false;
                ...
                // 删除提问
                if(_target.hasClass('question_delete')){
                    _qid = _target[0].getAttribute("data-id");
                    dataObject.access_token = window.access_token;
                    dataObject.action = 'delquestion';
                    dataObject.qid = _qid;
                    dataObject.getCount = 1;
                    $.ajax({
                        url: "//open.talk-fun.com/live/interaction.php",
                        dataType: "json",
                        data:"GET",
                        data:dataObject,
                        success: function(res){
                            if(res.code === 0){
                                // 删除当前回复
                                var id = "#queManage_"+_qid;
                                $(id).remove()
                                if(res.count <= 8){
                                    $(".quest_page").addClass('hidden');
                                }
                            }
                        }
                    })
                    // _HT.emit('question:delete',dataObject, function(res){
                    //     console.log(res)
                    // });
                };

                //通过提问，显示出来，
                if(_target.hasClass('question_pass')){
                    _qid = _target[0].getAttribute("data-id");
                    qid = $(this).data('id');
                    dataObject.access_token = window.access_token;
                    ...
                    // _HT.emit('question:pass',dataObject, function(res){
                    //     console.log(res)
                    // });
                };

                // 刷新列表（请求第一页数据）
                if(_target.hasClass('comfrim_refresh')){
                    if(MTSDK.admin.adminBox){
                        $(".quest_page").find("input").val("");
                        // 重置当前页数为1
                        pagination.page.pnum = 1;
                        MTSDK.admin.adminBox.questionManagement();
                    }
                }
        ...
        emitQuesManagement: function(){
            var $ques = $('.question_post'),
                $quesCon = $("#question_post_detail"),
                $quesVal = $.trim($quesCon.val()),
                // $quesHall = $("#question_hall"),
                _HT = HTSDK.room._HT,
                action = MT.getLiveState(),
                that = this,
                type = $(".comfirm_question_post").attr("data-type"),
                rid = $(".comfirm_question_post").attr("data-rid"),
            ...
            if(type === "ask"){
                $.ajax({
                    ...
                    success: function(res){
                        if(res.code === 0){
                            if(MTSDK.admin.adminBox.addAsk){
                                MTSDK.admin.adminBox.addAsk(res.data);
                            }
                        }
                    }
                })
                ...
                //     clearInterval(HTSDK.modQuestion.defaults.setTime);
                //     that.timeLimit(that.defaults.sendTimeLimit);
        ...
        timeLimit: function(time){
            var that = this;
            HTSDK.modQuestion.defaults.setTime = setInterval(function(){
                time-- ;
                that.defaults.sendTimeLimit = time;
         ...
         //老师端通过后,助教端应移除通过
        removePassBtn: function (retval) {
            $("#pass_"+retval.qid).remove();
        },
        ...
        renderQueslist: function(ret, scrollType){
            ...
            for (var i in d) {
                ...
                d[i].content = HTSDK.tools.textUrlLink(d[i].content);
                
                if(d[i].answer){
                    for(var j = 0 ; j < d[i].answer.length; j++){
                        d[i].answer[j].content = HTSDK.tools.textUrlLink(d[i].answer[j].content);
                    }
                }
        ...
        getQuestionList: function(){
            ...
            _HT.getQuestion(function(retval){
                if(retval.code === 0 && typeof retval.data !== "undefined"){
                    ...
                    // MTSDK.admin.adminBox.questionEvents("question:initialization", retval);
                }
            });
        },
        ...
        ask: function(ret,audit){
            ...
            d.content = HTSDK.tools.textUrlLink(ret.content); 
